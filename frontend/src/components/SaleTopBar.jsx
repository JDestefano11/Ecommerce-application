...
const SaleTopBar = () => {
  ...
  const [countdown, setCountdown] = useState(() => {
    ...
    if (savedCountdown) {
      ...
      const timeLeft =
        ...
        Math.floor(
          (Date.now() - parseInt(localStorage.getItem("lastUpdated"))) / 1000
        );
  ...
  useEffect(() => {
    ...
    const countdownInterval = setInterval(() => {
      setCountdown((prevCountdown) => {
        const newCountdown =
          prevCountdown > 0 ? prevCountdown - 1 : 24 * 60 * 60;
  ...
  return (
    <div className="fixed top-0 left-0 w-full bg-[#FFC0CB] text-[#2F4F4F] py-2 text-center font-bold z-50">
