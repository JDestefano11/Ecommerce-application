...
const PlaceOrder = () => {
  ...
  const calculateSubtotal = () => {
    return Object.entries(cartItems).reduce((total, [itemId, sizes]) => {
      ...
      const itemTotal = Object.values(sizes).reduce((sum, quantity) => sum + quantity * product.price, 0);
    ...
    <div className="flex flex-col sm:flex-row justify-between gap-8 pt-5 sm:pt-14 min-h-[80vh] border-t ">
      <div className="flex flex-col gap-6 w-full sm:w-1/2">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              {paymentMethods.map((method, index) => (
                <label key={index} className="flex items-center space-x-3 cursor-pointer">
                  ...
                  />
                  <img src={method.icon} alt={`Payment method ${index + 1}`} className="h-8" />
                </label>
              ))}
            </div>
          ...
          {paymentMethod !== "" && (
            <div className="space-y-2">
              <h2 className="text-lg font-semibold text-[#2F4F4F]">Payment Information</h2>
              <input
                type="text"
                name="cardNumber"
          ...
          {/* Submit Button */}
          <button type="submit" className="w-full bg-[#708090] text-white py-2 px-4 rounded-md hover:bg-[#FFC0CB] hover:text-[#2F4F4F] transition duration-300">
            Place Order
          </button>
        </form>
      </div>
      ...
      <div className="w-full sm:w-1/2 bg-[#FFFFFF] p-6 rounded-lg">
        <h2 className="text-xl font-semibold mb-4 text-[#2F4F4F]">Order Summary</h2>
