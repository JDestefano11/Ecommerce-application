...
const Profile = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-8 bg-[#FFFDD0] min-h-screen">
      <div className="max-w-3xl mx-auto bg-white shadow-lg rounded-lg overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {Object.entries(editedUser).map(([key, value]) => (
              <div key={key} className="mb-4">
                <label className="block text-[#708090] text-sm font-bold mb-2 capitalize">{key}</label>
                {isEditing ? (
                  <input
                    type="text"
                    name={key}
                  ...
                  <p className="text-[#2F4F4F] bg-[#FFFDD0] p-2 rounded-md">{value}</p>
                )}
              </div>
            ))}
          </div>
        ...
        <div className="mt-8 p-6 border-t border-gray-200">
          <h2 className="text-2xl font-semibold text-[#2F4F4F] mb-4">Recent Orders</h2>
          ...
          <div className="mt-4">
            <Link to="/orders" className="text-[#708090] hover:text-[#FFC0CB]">View All Orders</Link>
          </div>
        </div>
      </div>
